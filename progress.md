...
## 1.27.0 — the header stops lying, and the shell stops being 2024 (2026-09-06)

LO: "continue other improvemtns now." An adversarial audit of the whole
app — finders fanned out, then three independent skeptics per finding,
each prompted to REFUTE — confirmed exactly one thing, and it was the
kind that matters here.

- **The header printed a number it never measured.** `slot` was
  `285_000_000 + (now - genesis)/400 + hash32(minute) % 149`: a formula in
  the simulator, wearing a pulsing live dot and the tooltip "latest Solana
  slot this tab has seen", an inch from a SOL price that really is
  cross-checked across two APIs. The house rule calls a placeholder that
  looks like a measurement a bug, and this was the purest example in the
  codebase. No keyless source reports the head slot without a subscription
  nobody asked for, so the header is down to the price — and `slot` is
  gone from `MarketState` and from the store method that computed it, so
  it cannot be wired back by accident.
- **Three pages the rail files under SIMULATED did not say so.** Research
  stamps every answer "demo-universe (synthetic)" — under the answer, in
  10px grey, after it has been read. Backtest Lab says "the market itself
  is generated by this program" in the third clause of a faint line. 3D
  Network, a full-bleed scene with no title row, said nothing at all. All
  three carry the chip now; the scene's is pinned bottom-left, where the
  HUD's own toggle cannot hide it and clear of the time-machine bar at
  375px (measured on both viewports).
- **The disclaimer had drifted the other way.** It listed "wallet
  activity, wallet PnL" as still simulated. That stopped being true when
  the ledger landed: a pasted address is read off the chain and its
  realized PnL measured over a stated coverage window. The section's own
  comment warns that calling real data fake is the same drift as the
  opposite one — it had drifted that way itself. Corrected, and the
  correction is on the page.
- **The desktop shell had no Content-Security-Policy.** Chromium enforces
  nothing on a custom scheme by default, so a string that escaped React's
  escaping could have pulled a script off any host — and this app renders
  token names, symbols and wallet labels fetched from public APIs.
  Documents now carry a policy that blocks remote script, objects, framing
  and a rewritten `<base>`. Inline script it cannot block: Next's static
  export inlines its own bootstrap and has no server to mint a nonce. The
  export references no external script or stylesheet at all (checked), and
  three runs of the real shell logged zero violations.
- **`resolveStatic` threw on a malformed escape.** `decodeURIComponent`
  raises URIError on `%zz` or a trailing `%`, inside `protocol.handle`,
  where a rejection is not a 404 but a dead request. It takes the 404 road
  now.
- **Electron 33.2 → 44.2.0**, builder 25 → 26.15.3, updater 6.3.9 → 6.8.9.
  33 left support in mid-2025; every Chromium fix since — a CVE batch a
  month — was missing from a shipped desktop app. `main.js` needed no
  changes: `protocol.handle`, `net.fetch`, `before-input-event`,
  `screen.getAllDisplays` and the sandboxed window are all unchanged.
  Proven BEFORE tagging: unpackaged shell clean, builder 26 packages the
  same config untouched, and the packaged build reached the real release
  feed. The installer costs 79 MB → 108 MB; that is the price.

Tests 919, unchanged — none of this has a seam a unit test could hold.

| check | result |
|---|---|
| installer | `3ce4d81a…5771` = SHA256SUMS = GitHub digest; latest.yml 1.27.0, 113,340,407 bytes |
| CI | packaged `electron=44.2.0`; typecheck, 919 tests, static build, publish all green in 4m22s |
| site | `dee4f64`, Pages built, 3 anchors at 1.27.0, size corrected to 108 MB, `og.png` 200 live |
| desktop | 1.27.0.0 installed; CSP, `notFoundPage`, bridge and window-state in app.asar; window titled "ROM Nova — Solana On-Chain Intelligence"; leveldb LOG 11:45; updater answered "1.27.0 is not available (latest 1.27.0)" against the real feed; clean close (4 processes → 0, no force-kill); `window-state.json` rewritten on the close path at 11:46 |
| pane | SIMULATED chips on Research, Backtest and Network at 1586px and 375px; header shows SOL only |

Still open, not done: the release workflow pins `actions/checkout@v4` and
`actions/setup-node@v4`, both now forced onto Node 24 with a deprecation
warning. Latest is v7 for each, and setup-node v7 migrated to ESM — a jump
